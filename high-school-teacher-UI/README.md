# High School Management - Teacher UI

The teacher frontend for the High School Management System. It provides classroom, lesson, homework, quiz, attendance, grading, communication, and teacher dashboard workflows.
...
Start the teacher UI in another terminal:

```bash
npm run dev:teacher
```

The teacher UI runs at `http://localhost:3000` and the API runs at `http://localhost:5000`.
...
```bash
VITE_API_PROXY_TARGET=http://localhost:5000 npm run dev:teacher
```
...
```bash
VITE_USE_MOCK_API=true npm run dev:teacher
```
...
- `/teacher/classes` - assigned classes and students
- `/teacher/lessons` - lesson workflows
- `/teacher/homework` - homework workflows
- `/teacher/quizzes` - quiz workflows
- `/teacher/grades` - grade workflows
- `/teacher/attendance` - attendance workflows
