  ...
  return (
    <section className="rounded-[28px] glass-sm p-6 min-h-90">
      ...
      <ul className="mt-6 space-y-3">
        {enrollmentData.map((slice) => (
          <li key={slice.grade} className="flex items-center gap-3 text-sm text-slate-600">
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        ...
        />
        <span className="font-semibold text-slate-900">{entry.grade}</span>
      </div>
      <div className="mt-1 text-slate-600">
        {entry.count} students <span className="text-slate-500">({percent}%)</span>
      </div>
    </div>
  )
}