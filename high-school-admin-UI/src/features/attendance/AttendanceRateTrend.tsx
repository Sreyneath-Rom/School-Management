// src/features/attendance/AttendanceRateTrend.tsx
import {
  ...
  return (
    <section className="rounded-[28px] glass-sm p-6 min-h-90">
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h2 className="text-base font-semibold text-fg">Attendance Rate Trend</h2>
          <p className="text-sm text-fg-muted">This Week</p>
        </div>
        <div className="inline-flex items-center gap-3 rounded-full px-4 py-2 text-sm font-semibold text-fg shadow-sunken">
      ...
      <div className="h-80 w-full">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={attendanceTrendData} margin={{ top: 24, right: 12, left: -8, bottom: 0 }}>
            ...
            <CartesianGrid vertical={false} stroke="var(--neu-shadow-dark)" />
            <XAxis dataKey="day" axisLine={false} tickLine={false} tick={{ fill: 'var(--text-color-secondary)', fontSize: 12 }} dy={8} />
            <YAxis
              domain={[0, 100]}
      ...
      {breakdown && (
        <div className="mt-5 grid gap-3 sm:grid-cols-3">
          {breakdown.map((item) => (
            <div key={item.status} className="rounded-3xl p-4 text-sm text-fg-muted shadow-sunken">
              <div className="font-semibold uppercase tracking-[0.18em] text-fg-muted">{item.status}</div>
              <div className="mt-2 text-2xl font-semibold text-fg">{item._count}</div>
            </div>
          ))}
        </div>
      )}
...
function CustomTooltip({
  ...
  return (
    <div className="dropdown-surface rounded-2xl px-3 py-2 text-[13px]">
      ...
      <div className="mt-1 text-fg-muted">Attendance: {value}%</div>
    </div>
  )
}