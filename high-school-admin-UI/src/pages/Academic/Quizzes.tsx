...
export default function QuizzesPage() {
  ...
  const handleAddQuestion = () => {
    setForm((p) => ({
      ...
      questions: [
        ...p.questions,
        { ...EMPTY_QUESTION, id: `q-${Date.now()}-${p.questions.length}` },
      ],
    }))
  }
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    for (const q of form.questions) {
      if (!q.question.trim() || q.options.some((o) => !o.trim())) {
        showToast('Every question needs text and all options filled', 'error')
        return
      }
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="glass-sm rounded-2xl p-4 border border-surface">
      ...
      {loading ? (
        ...
        <div className="glass-sm rounded-2xl p-12 text-center border border-surface">
          ...
          <h3 className="text-base font-semibold text-color">No quizzes</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
          {filtered.map((q) => (
            ...
            >
              ...
              <div className="mt-4 pt-3 border-t border-surface flex items-center justify-end">
                {isStudent ? (
                  <button
                    onClick={() => startQuiz(q)}
      ...
      {activeQuiz && activeQuiz.questions[currentIdx] && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
          <div className="bg-surface rounded-3xl max-w-3xl w-full max-h-[95vh] flex flex-col shadow-2xl border border-surface overflow-hidden">
            <div className="p-5 border-b border-surface flex items-center justify-between bg-surface-strong">
              <div>
                ...
                <h3 className="font-bold text-color text-base mt-1">
                  {activeQuiz.title}
                </h3>
              </div>
            ...
            <div className="px-6 py-3 border-b border-surface flex items-center gap-2 overflow-x-auto">
            ...
            <div className="p-6 flex-1 overflow-y-auto space-y-6">
              ...
              <h4 className="text-base font-semibold text-color leading-relaxed">
                    ...
                    <div
                      ...
                      className={`p-4 rounded-2xl border transition cursor-pointer flex items-center gap-3.5 ${
                        selected
                          ...
                          : 'bg-surface-strong border-surface hover:border-brand-500/40'
                      }`}
                    >
                      <div
            ...
            <div className="p-4 border-t border-surface flex items-center justify-between bg-surface-strong">
              <button
                disabled={currentIdx === 0}
                onClick={() => setCurrentIdx((p) => Math.max(0, p - 1))}
                className="px-4 py-2 rounded-xl text-xs font-medium text-color bg-surface border border-surface disabled:opacity-40 inline-flex items-center gap-1"
              >
              ...
              {currentIdx < activeQuiz.questions.length - 1 ? (
                <button
                  onClick={() =>
                    setCurrentIdx((p) =>
                      Math.min(activeQuiz.questions.length - 1, p + 1)
                    )
                  }
                  className="px-4 py-2 rounded-xl text-xs font-medium bg-brand-600 text-white hover:bg-brand-700 inline-flex items-center gap-1"
                >
                ...
                <button
                  ...
                  className="px-5 py-2 rounded-xl text-xs font-medium bg-success hover:opacity-90 text-white shadow-sm inline-flex items-center gap-1.5"
                >
      ...
      {result && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-md">
          <div className="bg-surface rounded-3xl max-w-md w-full p-6 shadow-2xl border border-surface space-y-5 text-center">
            <div className="w-16 h-16 rounded-3xl mx-auto flex items-center justify-center bg-brand-500/15 text-brand-600">
              <Award className="w-8 h-8" />
            </div>
            <div>
              <h2 className="text-xl font-bold text-color">Quiz Complete</h2>
              <p className="text-sm text-secondary mt-1">{result.quiz.title}</p>
            </div>
            <div className="text-3xl font-extrabold text-color">
              {result.submission.score} / {result.submission.maxScore}
            </div>
            <button
              onClick={() => setResult(null)}
              className="px-5 py-2.5 rounded-xl text-sm font-medium bg-brand-600 hover:bg-brand-700 text-white"
            >
              Close
            </button>
          </div>
      ...
      {reviewQuiz && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-surface rounded-3xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6 shadow-2xl border border-surface space-y-5">
            ...
            <div className="space-y-4">
              {reviewQuiz.questions.map((q, idx) => (
                ...
                >
                  ...
                  <p className="text-sm font-medium text-color">{q.question}</p>
                    ...
                    {q.options.map((opt, oi) => (
                      <div
                        ...
                        className={`p-2.5 rounded-xl border ${
                          oi === q.correctAnswer
                            ? 'bg-success/10 border-success/40 font-semibold text-success'
                            : 'bg-surface border-surface text-color'
                        }`}
                      >
      ...
      {isCreateOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <form
            ...
            className="bg-surface rounded-3xl max-w-3xl w-full max-h-[90vh] overflow-y-auto p-6 shadow-2xl border border-surface space-y-5"
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <label className="flex items-center gap-2 text-xs text-color self-end pb-2">
                <input
                  type="checkbox"
                  checked={form.isAutoGrade}
                  onChange={(e) =>
                    setForm({ ...form, isAutoGrade: e.target.checked })
                  }
                />
            ...
            <div className="space-y-4 pt-2">
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-bold text-color">
                  Questions ({form.questions.length})
                </h3>
                <button
                  type="button"
                  onClick={handleAddQuestion}
                  className="text-xs px-3 py-1.5 rounded-xl font-medium bg-surface hover:bg-surface-strong text-color inline-flex items-center gap-1"
                >
              ...
              {form.questions.map((q, qi) => (
                <div
                  key={q.id}
                  className="p-4 rounded-2xl border border-surface bg-surface-strong space-y-3"
                >
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        value={q.points}
                        onChange={(e) =>
                          handleUpdateQuestion(qi, {
                            points: Number(e.target.value),
                          })
                        }
                        className="w-16 px-2 py-1 text-xs rounded-lg border border-surface bg-surface text-color text-center font-semibold"
                      />
                      ...
                      {form.questions.length > 1 && (
                        <button
                          ...
                          className="p-1 rounded text-secondary hover:text-error"
                        >
                          <X className="w-4 h-4" />
                        </button>
                      )}
                  ...
                  <input
                    ...
                    className="w-full px-3 py-1.5 text-xs rounded-xl border border-surface bg-surface text-color"
                  />
                    ...
                    {q.options.map((opt, oi) => (
                      <div key={oi} className="flex items-center gap-2">
                        <input
                          ...
                          onChange={() =>
                            handleUpdateQuestion(qi, { correctAnswer: oi })
                          }
                          className="text-brand-600"
                        />
                        <span className="text-xs font-bold text-secondary w-4">
                        ...
                        <input
                          ...
                          value={opt}
                          onChange={(e) =>
                            handleUpdateOption(qi, oi, e.target.value)
                          }
                          className="flex-1 px-3 py-1 text-xs rounded-lg border border-surface bg-surface text-color"
                        />
                      </div>
                    ))}
                  </div>
