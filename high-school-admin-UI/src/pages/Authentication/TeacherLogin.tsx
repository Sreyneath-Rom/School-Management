...
export default function TeacherLogin() {
  ...
  const { values, errors, handleChange, handleBlur, handleSubmit, setFieldValue } =
    useForm<TeacherLoginForm>(
      ...
      async (formValues: TeacherLoginForm) => {
        ...
        setError('');
        try {
          const result = await authService.login(formValues.teacherId.trim(), formValues.password);
      ...
      (formValues) => {
        ...
        if (!formValues.teacherId.trim()) {
          validationErrors.teacherId = 'Teacher ID or Email is required';
        }
  ...
  const displayName = getUserGreeting(values.teacherId);

  const handleQuickFill = () => {
    setFieldValue('teacherId', 'teacher@example.com');
  ...
  return (
    <div className="min-h-screen flex flex-col justify-between relative overflow-hidden bg-slate-50/60 dark:bg-slate-950 font-sans">
          ...
          <div className="lg:col-span-5 flex flex-col justify-between space-y-6 text-slate-800 dark:text-slate-200">
            ...
            <div className="p-4 rounded-2xl bg-emerald-50/80 dark:bg-emerald-950/40 border border-emerald-200/80 dark:border-emerald-800/60">
              ...
              <div className="text-xs font-mono text-slate-700 dark:text-slate-300 space-y-0.5">
                <p>Email / ID: <strong className="text-slate-900 dark:text-white">teacher@example.com</strong> or <strong className="text-slate-900 dark:text-white">TCH-123456</strong></p>
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500 dark:text-slate-400">
                      {displayName ? (
                        ...
                        'Enter your Teacher ID or school email to begin.'
                      )}
                    </p>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      type="text"
                      name="teacherId"
                      placeholder="teacher@example.com or TCH-123456"
                  ...
                  {errors.teacherId && <p className="text-red-500 text-xs mt-1.5 pl-1">{errors.teacherId}</p>}
                </div>
      ...
      <ForgotPasswordModal
        ...
        roleName="Teacher"
        defaultIdentifier={values.teacherId}
      />
    </div>
  );
}
