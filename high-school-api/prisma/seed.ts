...
async function main() {
  ...
  await grantAll(adminRole.id, permissions.map((p: Permission) => p.id))
  ...
  const teacherPermissionIds = permissions
    .filter((p: Permission) => teacherModules.some((m) => p.key.startsWith(`${m}.`)))
  ...
  const studentPermissionIds = permissions
    .filter((p: Permission) => studentModules.some((m) => p.key === `${m}.view`) || p.key === 'homework.create' || p.key === 'quizzes.create' || p.key === 'leaveRequests.create')
    .map((p: Permission) => p.id)
